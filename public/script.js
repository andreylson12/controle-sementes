...
function toKg(qty, unit){
  const perSC = SETTINGS?.units?.kg_per_sc || 60;
  const perBag = SETTINGS?.units?.kg_per_bag || 1000;
  if(unit==="kg") return Number(qty||0);
  if(unit==="sc") return Number(qty||0) * perSC;
  if(unit==="bag") return Number(qty||0) * perBag;
  return Number(qty||0);
}
...
async function loadLotes(){
  ...
  // preencher selects com saldo em bag
...
async function loadTrat(){
  ...
  const elSum = $("#tratSum"); if (elSum) elSum.textContent = `${fmt(toBag(totalKg))} bag (${fmt(totalKg)} kg)`;
  const elCnt = $("#tratCount"); if (elCnt) elCnt.textContent = filtered.length;
...
async function loadMov(){
  ...
  const filtered = all.filter(m => {
    ...
    if (qLot && !lotName.includes(qLot)) return false;
    const dest = `${m.destination_type || ""} ${m.destination_name || ""}`.toLowerCase();
...
async function loadDashboard(){
  // filtros
  const qVar  = ($("#fDashVar")?.value || "").trim().toLowerCase();
  const from  = $("#fDashFrom")?.value ? new Date($("#fDashFrom").value) : null;
  const to    = $("#fDashTo")?.value ? new Date($("#fDashTo").value)   : null;

  const lots = await api("/api/seed-lots");
  const trts = await api("/api/treatments");
  const movs = await api("/api/movements");

  // Entradas no período (considera unit do cadastro)
  const entradasKg = lots
    .filter(l=>{
      if(qVar && !(l.variety||"").toLowerCase().includes(qVar)) return false;
      const d = l.received_at ? new Date(l.received_at) : null;
      if(from && (!d || d < from)) return false;
      if(to   && (!d || d > to))   return false;
      return true;
    })
    .reduce((s,l)=> s + toKg(l.qty, l.unit), 0);

  // Tratado no período
  const tratadoKg = trts
    .filter(t=>{
      const lotName = (t.lot_name || LOT_INDEX[t.lot_id] || "").toLowerCase();
      if(qVar && !lotName.includes(qVar)) return false;
      const d = t.treated_at ? new Date(t.treated_at) : null;
      if(from && (!d || d < from)) return false;
      if(to   && (!d || d > to))   return false;
      return true;
    })
    .reduce((s,t)=> s + Number(t.qty_kg||0), 0);

  // Saídas no período
  const names = {}; lots.forEach(l => names[l.id] = `${l.variety||""} • ${l.lot_code||l.id}`);
  const saidasKg = movs
    .filter(m=>{
      const lotName = (names[m.lot_id] || "").toLowerCase();
      if(qVar && !lotName.includes(qVar)) return false;
      const d = m.moved_at ? new Date(m.moved_at) : null;
      if(from && (!d || d < from)) return false;
      if(to   && (!d || d > to))   return false;
    ...
    .reduce((s,m)=> s + Number(m.qty_kg||0), 0);

  // Não tratado (período) = Entradas - Tratado
  const naoTratadoKg = Math.max(0, entradasKg - tratadoKg);

  // Estoque atual (ignora período; aplica filtro de variedade se quiser)
  const estoqueKg = lots
    .filter(l=> !qVar || (l.variety||"").toLowerCase().includes(qVar))
    .reduce((s,l)=> s + Number(l.balance_kg||0), 0);

  // Preenche KPIs
  $("#kpiEntradasKg").textContent = `${fmt(entradasKg)} kg`;
  $("#kpiEntradasBag").textContent = `${fmt(toBag(entradasKg))} bag`;

  $("#kpiTratKg").textContent = `${fmt(tratadoKg)} kg`;
  $("#kpiTratBag").textContent = `${fmt(toBag(tratadoKg))} bag`;

  $("#kpiNaoTratKg").textContent = `${fmt(naoTratadoKg)} kg`;
  $("#kpiNaoTratBag").textContent = `${fmt(toBag(naoTratadoKg))} bag`;

  $("#kpiSaidasKg").textContent = `${fmt(saidasKg)} kg`;
  $("#kpiSaidasBag").textContent = `${fmt(toBag(saidasKg))} bag`;

  $("#kpiEstoqueKg").textContent = `${fmt(estoqueKg)} kg`;
  $("#kpiEstoqueBag").textContent = `${fmt(toBag(estoqueKg))} bag`;
}
...
async function generateDashboardPDF(){
  const css=`<style>
    ...
    .ttl{font-size:12px;color:#666;margin:0 0 4px}
    .val{font-size:18px;font-weight:700;margin:0}
    .sub{font-size:12px;color:#666}
  </style>`;
  const now = new Date().toLocaleString();
  const w = window.open("", "_blank");
  w.document.write(`<!doctype html><html><head><meta charset="utf-8">${css}</head><body>
    <h1>Resumo Executivo — Controle de Sementes</h1>
    <div class="meta">Gerado em ${now}</div>
    <div class="kpis">
      ${["Entrou (período)","Tratado (período)","Não tratado (período)","Saídas (período)","Estoque Atual"]
        .map((title,i)=>{
          const ids=[
            ["kpiEntradasKg","kpiEntradasBag"],
            ["kpiTratKg","kpiTratBag"],
            ["kpiNaoTratKg","kpiNaoTratBag"],
            ["kpiSaidasKg","kpiSaidasBag"],
            ["kpiEstoqueKg","kpiEstoqueBag"],
          ][i];
          return `<div class="card"><div class="ttl">${title}</div>
            <div class="val">${document.getElementById(ids[0]).textContent}</div>
            <div class="sub">${document.getElementById(ids[1]).textContent}</div></div>`;
        }).join("")}
    </div>
    <p class="meta">Obs: “Não tratado (período)” = Entradas (kg) − Tratado (kg) no período filtrado.</p>
  </body></html>`);
  w.document.close(); w.focus(); w.print();
}

// ===== Relatórios (Entradas/Saídas) já existentes =====
...
document.addEventListener("submit", async (e)=>{
  ...
  if(form.id==="formLote"){
    ...
    try{
      ...
      form.reset(); await Promise.all([loadLotes(),loadEstoque(),loadDashboard()]); alert("Lote salvo com sucesso!");
  ...
  if(form.id==="formTrat"){
    ...
    try{
      ...
      form.reset(); await Promise.all([loadTrat(),loadLotes(),loadEstoque(),loadDashboard()]); alert("Tratamento registrado!");
    }catch(err){ try{alert(JSON.parse(err.message).message||err.message);}catch{alert(err.message);} }
    return;
  }
  ...
  if(form.id==="formMov"){
    ...
    try{
      ...
      form.reset(); await Promise.all([loadMov(),loadLotes(),loadEstoque(),loadDashboard()]); alert("Saída registrada!");
    }catch(err){ try{alert(JSON.parse(err.message).message||err.message);}catch{alert(err.message);} }
  }
});
...
$("#formCfg")?.addEventListener("submit", async (e)=>{
  ...
  await Promise.all([loadCfg(),loadLotes(),loadEstoque(),loadTrat(),loadMov(),loadDashboard()]);
...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  ["fTratLot","fTratProd","fTratFrom","fTratTo"].forEach(id=>{
    const el=document.getElementById(id); if(!el) return;
    el.addEventListener(el.type==="date" ? "change":"input", ()=>loadTrat());
  });
  $("#btnTratFiltrar")?.addEventListener("click",()=>loadTrat());
  $("#btnTratLimpar")?.addEventListener("click",()=>{
    ["fTratLot","fTratProd","fTratFrom","fTratTo"].forEach(id=>{ const el=$("#"+id); if(!el) return; el.value=""; });
    loadTrat();
  });
  ...
  ["fMovLot","fMovDest","fMovFrom","fMovTo"].forEach(id=>{
    const el=document.getElementById(id); if(!el) return;
    el.addEventListener(el.type==="date" ? "change":"input", ()=>loadMov());
  });
  $("#btnMovFiltrar")?.addEventListener("click",()=>loadMov());
  $("#btnMovLimpar")?.addEventListener("click",()=>{
    ["fMovLot","fMovDest","fMovFrom","fMovTo"].forEach(id=>{ const el=$("#"+id); if(!el) return; el.value=""; });
    loadMov();
  });

  // filtros dashboard
  ["fDashVar","fDashFrom","fDashTo"].forEach(id=>{
    const el=document.getElementById(id); if(!el) return;
    el.addEventListener(el.type==="date" ? "change":"input", ()=>loadDashboard());
  });
  $("#btnDashFiltrar")?.addEventListener("click",()=>loadDashboard());
  $("#btnDashLimpar")?.addEventListener("click",()=>{
    ["fDashVar","fDashFrom","fDashTo"].forEach(id=>{ const el=$("#"+id); if(!el) return; el.value=""; });
    loadDashboard();
  });
  $("#btnDashPDF")?.addEventListener("click",()=>generateDashboardPDF());
...
const firstTab=(location.hash||"").replace("#","")||"dash";
